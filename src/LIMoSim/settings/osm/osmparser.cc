...
void OSMParser::parseRelation(DOMElement *_relation)
{
    ...
    {
        for(unsigned int i=0; i<_relation->childNodes.size(); i++)
        {
            DOMElement *child = _relation->childNodes.at(i)->toElement();
            if(child->tagName=="member")
            {
                ...
                if(role=="outer") // TODO: we do not parse multipolygons here, just the outline is used to define the building
                {
                    ...
                    Building *building = p_world->createBuilding(id);
                    if(m_ways.count(ref)>0)
                    {
                        DOMElement *way = m_ways[ref];
                        for(unsigned int j=0; j<way->childNodes.size(); j++)
                        {
                            DOMElement *child = way->childNodes.at(j)->toElement();
                            if(child->tagName=="nd")
                            {
...
Road* OSMParser::parseRoad(const std::string &_id, int _type, std::map<std::string, Variant> &_tags)
{
    ...
    if(_type==ROAD_TYPE::MOTORWAY || _type==ROAD_TYPE::MOTORWAY_LINK || _type==ROAD_TYPE::MOTORWAY_JUNCTION || _type==ROAD_TYPE::TRUNK)
    {
        road->setForwardLanes(1);
        road->setBackwardLanes(0);
    }
    else
    {
        road->setForwardLanes(1);
        road->setBackwardLanes(std::max(road->getBackwardLanes(), 1));
    }
