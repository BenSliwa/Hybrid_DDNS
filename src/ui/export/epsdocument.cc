...
{
    startPath();
    m_data += point(_position) + " " + QString::number(_radius) + " 0 360 arc ";
    closePath();
    setColor(_color);
    fill();
}
...
void EpsDocument::drawEllipse(const QPointF &_center, double _width, double _height, double _rotation, const QColor &_color)
{
    m_data += "q\n"; // save the context for rotation
    m_data += QString::number(_center.x()) + " " + QString::number(_center.y()) + " translate\n";

    startPath();
    rotate(_rotation);
    m_data += "1 " + QString::number(_height/_width) + " scale\n";
    m_data += "0 0 " + QString::number(_width) + " 0 360 arc ";
...
{
    m_data += QString::number(_width) + " setlinewidth\n";
}
...
void EpsDocument::rotate(double _angle)
{
    m_data += QString::number(_angle) + " rotate\n";
}
...
QString EpsDocument::point(const QPointF &_position)
{
    return QString::number(_position.x()) + " " + QString::number(_position.y());
}
}
